...
class HuagoConeReleaseDefinitionTests(unittest.TestCase):
    def test_eval_image_pins_agent_versions(self):
        text = (Path(__file__).parents[1] / "deploy" / "docker" / "Dockerfile.agent-eval").read_text(encoding="utf-8")
    ...
    def test_compose_is_read_only_and_single_concurrency(self):
        text = (Path(__file__).parents[1] / "deploy" / "docker" / "compose.yaml").read_text(encoding="utf-8")
    ...
    def test_release_runner_never_copies_auth_or_secret_into_context(self):
        text = (Path(__file__).parents[1] / "Test" / "run_v3_huago_cone_release.py").read_text(encoding="utf-8")
