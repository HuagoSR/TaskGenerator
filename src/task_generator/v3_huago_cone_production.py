...
from .v3_hybrid_task_materializer import HybridTaskMaterializer
from .v3_task_design_executor import (
    TaskDesignExecutionReportV1,
    TaskDesignExecutionRequestV1,
    TaskDesignProposalExecutor,
)
from .v3_task_design_frontend import (
    CapabilityBriefV1,
    CapabilityTargetV1,
    DesignSourceRefV1,
...
from .v3_skill_extractor import ExtractedSkillCandidate, ProviderConfig
...
class ProductionModelComparisonResultV1(BaseModel):
    model_config = ConfigDict(extra="forbid")
    result_version: Literal["v3.production_model_comparison.1"] = (
        "v3.production_model_comparison.1"
    )
    ...
    discrimination_decision: Literal[
        "usefully_discriminative", "low_model_separation", "insufficient"
    ]
...
def aggregate_production_model_comparison(
    records: List[ModelTaskEvaluationV1],
) -> ProductionModelComparisonResultV1:
    solver_ids = [
        "gpt-5.6-sol@chatgpt_codex",
        "deepseek-v4-pro@official_opencode",
        "gemini-3.1-pro-preview@tuzi_opencode",
    ...
    for solver_id in solver_ids:
        ...
        for item in values:
            ...
            if item.delivery_valid and len(valid_judges) == 2:
                ...
                if abs(float(valid_judges[0].weighted_score) - float(valid_judges[1].weighted_score)) >= 0.15 or major_values[0] != major_values[1]:
                    disagreements += 1
        summaries.append(
            ModelSummaryV1(
                solver_id=solver_id,
                valid_delivery_count=sum(item.delivery_valid for item in values),
                dual_graded_count=len(dual),
                major_defect_count=majors,
                mean_composite_score=(sum(dual) / len(dual) if dual else None),
                comparison_eligible=len(dual) >= 8,
            )
        )
    eligible = [item.solver_id for item in summaries if item.comparison_eligible]
    task_sets = [
        {task for solver, task in composite if solver == solver_id}
        for solver_id in eligible
    ]
    ...
    for task_id in task_ids:
        ...
        level = "high" if spread >= 0.15 else "medium" if spread >= 0.08 else "low"
        discrimination.append(TaskDiscriminationV1(task_id=task_id, model_count=len(scores), score_range=spread, level=level))
    complete_discrimination = [item for item in discrimination if item.model_count == 3 and item.score_range is not None]
    useful_count = sum(item.level in {"medium", "high"} for item in complete_discrimination)
    mean_range = (
        sum(float(item.score_range) for item in complete_discrimination) / len(complete_discrimination)
        if complete_discrimination
        else 0.0
    )
    discrimination_decision = (
        "insufficient"
        if len(complete_discrimination) < 8
        else "usefully_discriminative"
        if useful_count >= 6 and mean_range >= 0.10
        else "low_model_separation"
    )
    decision = (
        "comparison_complete"
        if len(eligible) == 3 and len(common) >= 8
        else "comparison_partial"
        if len(eligible) >= 2 and len(common) >= 8
        else "comparison_incomplete"
    )
    return ProductionModelComparisonResultV1(
        model_summaries=summaries,
        common_task_count=len(common),
        judge_disagreement_count=disagreements,
        task_discrimination=discrimination,
        discrimination_decision=discrimination_decision,
        decision=decision,
    )
