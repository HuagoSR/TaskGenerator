...
当前开发优先采用 S1：一个强作者在同一会话中读取完整生成合同与按需 Professional Skill，使用工具和自检，提交 GDPval-shaped 候选包、教师监督、正式原子 rubric 与可重放计算依据，再进入共同的有限准入。历史多阶段 P 保留为参照。S1 优先是减少编排成本的实施选择，不改变 World-First、候选／教师隔离、正式 rubric 唯一性或外部实验另定 scope 的边界。

关键职业结果位于教师侧，用于说明正式 rubric 覆盖哪些高价值职业成果并支持行为诊断；它们不构成第二套评分。生成时允许把一个职业结果拆为多个独立可观察的 rubric item，但不得增加候选不可见义务、要求外化不必要的内部推理，或因下游连锁结果重复扣同一错误。证据状态、合理替代表达、记录来源因果和全格式候选可见性属于专业准入内容，不新增通用业务 schema 或运行时 evidence graph。
