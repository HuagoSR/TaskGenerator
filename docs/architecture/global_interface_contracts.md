...
`v3.production_substrate.1` is the run-scoped predecessor to `ProductionTaskCohortV1`. It binds the source manifest/tree, normalized visible-text source blocks, two domain-specific extraction prompt packages, redacted provider-attempt diagnostics, deterministic candidate-review results and a scratch registry. It requires at least three accepted, unique, source/block-grounded skills in each domain, records canonical registry hashes before/after, and fails if canonical state changes. It permits only Tuzi `gpt-5.6-sol`, zero SDK retries and one same-input retry for timeout, transport, 408/429/5xx, empty/truncated response, invalid JSON or schema failure; it has no DeepSeek, Codex or mock fallback authority.

For a new R9 production case, the second and final task-design attempt is mutually exclusive: it is either a same-brief provider retry for the listed classified provider failures, or a feedback-conditioned repair based on a persisted strict/semantic validation finding. Low-quality delivery, task score and materialization outcomes do not authorize redraw.
...
The first R9 production instance is frozen as `production_insufficient`: 7/10 package-ready and three provider-layer `InternalServerError` records after exactly ten normal calls. `v3.production_generation_result.1` correctly forbids treating those infrastructure failures as repairable validation findings. Because the result is below eight, `v3.production_model_comparison.1` has no admissible input and no probe, solver, grader or comparison evidence may be inferred. The seven admitted packages retain valid integrity evidence but cannot be supplemented with historical tasks.

The separate recovery instance is `production_ready` with 10/10 new blind tasks, ten first-attempt materializations and 39/39 valid candidate workbooks. It is homogeneous within its own campaign and may enter model comparison; the earlier seven packages remain excluded.

`v3.r9_solver_outcome.1` records one native Codex or OpenCode process, redacted stdout/stderr hashes, hard-timeout state and exact XLSX inspection. `v3.r9_solver_campaign.1` binds the recovery generation SHA, public-probe admission, per-task atomic state, cross-domain canary and three-consecutive-infrastructure freeze rule. Actual credential values are normalized in memory and removed from both streams before persistence; a redaction failure blocks evidence creation. These records authorize neither fallback adapters nor model-capability claims from infrastructure failures.

`v3.r9_judge_manifest.1` binds exactly one solver campaign, recovery generation result and judge identity. Only solver outcomes with exact valid XLSX are eligible. A judge receives candidate requirements, the frozen seven-dimension rubric, deterministic fact anchors and the delivery, but no solver or generation-route identity. Each eligible item has one substantive attempt; only invalid/empty/schema-nonconforming output may receive one format-conditioned attempt. Program code verifies blind task identity and recomputes weighted score and professional plausibility. Temporary Codex authentication links are process-scoped and removed before artifact persistence.

`v3.r9_evaluation_record_set.1` verifies all solver, judge, task and review hashes before compiling `ModelTaskEvaluationV1` inputs. It cannot turn missing or invalid delivery/grade evidence into a score. `v3.production_model_comparison.1` remains the sole aggregator and retains the fixed eight-task coverage, judge-disagreement and task-discrimination thresholds.

For server execution, artifact paths stored in R9 solver and judge manifests are container paths rooted at `/data/runs`. They are authoritative only when resolved inside the release container; the corresponding host persistence root is the compose mount backing that path. `compile-evaluations` must execute inside the pinned container so its hash checks resolve the recorded paths directly. Any out-of-container audit must make that mount mapping explicit and must not rewrite the manifest paths.
