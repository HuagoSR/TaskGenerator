...
A new homogeneous recovery campaign has now closed the production gap. It retained the frozen source/skill/brief substrate but generated ten new blind identities and reused none of the first campaign's proposals or packages. All 10/10 normal Tuzi calls materialized on attempt one, with zero repair and zero infrastructure failure. The ten packages and all 39 candidate workbooks pass the complete integrity audit. This `production_ready` recovery cohort is the sole R9 evaluation input.

Evaluation now uses `R9NativeSolverRunner`, a thin native CLI layer rather than another agent framework. It owns only isolated staging, Codex/OpenCode invocation, in-memory secret injection and redaction, hard timeout, canary/freeze state and XLSX admission. Full local regression passes 431/431. Final post-document parity and a replacement immutable release are required before private solver execution.

The staging fix is active in `milestone-r9-huago-cone-5d8f76146a38`: local and fixed parity pass 432/432, and all three public probes pass from fresh workspaces. The user has now explicitly authorized the ten recovery packages for all three named solver stacks and both named judges. The dual-judge runner and deterministic evaluation compiler pass the full 435-test local suite; a new post-document parity/release is the final gate before canary, full solving, dual grading and aggregation. No private result may be inferred from the earlier public probes.

R9 execution is now complete on the replacement release `milestone-r9-huago-cone-6c3ae7f21ba6` (`f5fa0da...`): its 435/435 local and fixed Linux/amd64 parity and smoke gates passed before activation. Each of GPT/Codex, official DeepSeek/OpenCode and Gemini/Tuzi/OpenCode passed a fresh public probe, the cross-domain canary and all ten recovery tasks. All 30 deliveries were exact valid XLSX and all 60 route-blind judge records completed; only JSON/schema output used the bounded formatting retry, never a low-score redraw. Hash-bound compilation produced `evaluation_records.json` SHA `df0be942...de84` and deterministic aggregation produced `production_model_comparison_result.json` SHA `2a3b4f0c...b9a3`.

The result is `comparison_complete` and `usefully_discriminative`: all stacks have 10 valid, dual-graded deliveries; composites are GPT `0.812500`, DeepSeek `0.767500`, Gemini `0.683125`, with major-defect counts `0/0/2`. GPT and DeepSeek differ by only `0.045`, below the fixed 0.05 materiality threshold, and therefore remain a practical tie; Gemini differs materially but has seven judge disagreements. This closes R9 as an internal engineering evaluation, not an expert-validity, default-solver, training, public-release or promotion decision.
