...
The staging fix is now active in `milestone-r9-huago-cone-5d8f76146a38`: local and fixed parity pass 432/432, and all three public probes pass from fresh workspaces. Private execution remains zero because the runtime permission reviewer requires an explicit authorization naming the recovery campaign and its three destinations. Solver, grading and comparison remain pending rather than inferred from public compatibility.
