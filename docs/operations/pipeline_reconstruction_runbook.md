...
当前允许的下一步是执行两题 task/truth compilation：新的 scope/receipt 绑定两份冻结有 Skill bundle、父 Bible、规则、Skill、Codex/Sol、镜像和一题一次正常编译加一次格式修正上限。两题静态通过后只生成用户检查包。不得自动扩展到 solver、grader、生产发布、训练或 registry mutation。
