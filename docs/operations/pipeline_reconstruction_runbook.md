...
R10.6 两题 task/truth compilation 已完成：第三个 scope/receipt 绑定两份冻结有 Skill bundle、父 Bible、规则、Skill、Codex/Sol 和镜像；两题均静态通过。Teacher Truth 事实必须由所列 candidate evidence 的 evidence map 投影支持；路径规范化不改变证据含义，初始 blocked 报告保留。用户检查包是当前唯一允许的后续读取动作。不得自动扩展到 solver、grader、生产发布、训练或 registry mutation。
