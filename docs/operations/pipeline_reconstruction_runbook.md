...
Timeout-enforcement resumption (2026-08-28): the user reopened the work after the r3 diagnostic. The old freeze still protects r1–r3 and their partial packages, but no longer prohibits a new reproduction. R4 confirmed that the same SDK behavior also leaves source-to-skill calls unbounded: its procurement extraction exceeded the configured 180 seconds and was stopped while its completed audit substrate remains diagnostic only. Before creating r5, every production provider exchange must use `max_retries=0` and, on Linux in the main production thread, a `SIGALRM` end-to-end deadline: 180 seconds for skill extraction and 900 seconds for task design. A deadline exception is classified under the existing retryable timeout path; it may consume only the pre-existing single same-input format/transport retry. It cannot create a repair prompt, reuse a partial package, or extend the call deadline. After source/test/parity/release checks, create a previously absent run root and repeat the entire source-to-package chain. Do not start solver or judge execution unless that new cohort is 10/10.

R9 production recovery (2026-08-29): replace the main-thread deadline with the existing Linux `fork` child exchange boundary. The parent must terminate then kill a child that outlives the frozen 180-second skill or 900-second task-design deadline; provider SDK retry remains zero. A classified `524`, timeout, 408/429/5xx, empty response, truncation or schema failure may consume the one existing same-input retry and must preserve the first diagnostic; a second failure freezes that work item with no third call. Release admission now requires at least **95 GiB** available disk space. The user authorized `docker builder prune -af` only for unused BuildKit cache; do not run `docker system prune`, remove containers/images/volumes, or touch resources outside TaskGenerator. Candidate release parity and both image smokes must pass before activation. Then create a previously absent R7 output root and run exactly `collect → build-substrate → compile-briefs → compile → generate`; do not reuse any R1–R6 source, skill, brief, proposal, package or result. Stop at a fresh 10/10 package-ready integrity report; this recovery does not authorize solver, grader or model-comparison execution.

R7 closure (2026-08-29): release `milestone-r9-huago-cone-746ff9b55f1b` passed 111/111 local and fixed Linux/amd64 network-disabled/read-only/no-credential parity plus factory/eval smoke, then became `current`; `6c3ae7...` remains `previous`. The allowed BuildKit-only cleanup left 123.7 GiB free before the build and 130 GiB after the frozen run; no container, image, volume or non-TaskGenerator project was removed. Fresh public collection, scratch-substrate review, brief compile and cohort compile passed. The generation container began 16 bounded provider attempts and persisted 15 execution reports: five packages materialized and passed their individual hybrid/export checks, four tasks exhausted their only controlled retry as provider failures, and the tenth task was persisted as `interrupted` after its first semantic-proposal block while its sole repair had begun, when three consecutive provider-side task failures triggered the campaign freeze. The frozen result is `r9_huago_cone_fresh_reproduction_10_20260829_r7`, manifest SHA `ca39961c...1a9d0`; it is a 5/10 provider-stability diagnostic, not a partial evaluation cohort. Do not resume it, run its solver/grader stages, mix it with any prior run or selectively regenerate its missing tasks. Before another production campaign, produce a report-first diagnosis of the saved redacted provider diagnostics and decide a minimal output-contract repair; any code repair requires a new immutable release and a brand-new source-to-package run root.
