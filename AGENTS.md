...
- F. Training-data readiness — finance production and the governed 30-task model-difference evaluation are complete; an 8-task diagnostic found task-contract and rubric revision is required before training.
...
Milestones D and E are complete. Milestone F finance production produced 60/60 candidate-ready tasks. The governed evaluation of 30 tasks is complete with 120/120 solver processes returned, 116 valid deliveries, 116 primary grades, 63 audit grades, four `gpt-4o-mini` non-deliveries, and no OOM or provider failure. A fixed 8-task human-style diagnostic then found all three task templates need candidate-visible contract repair and all eight sampled rubrics need fact-centered revision. The remaining 30 tasks are not authorized for evaluation. The next bounded work is to revise the contracts and rubrics, then validate a 3–5 task cohort; this does not authorize further external evaluation, RL training, default-chain promotion, or broad multi-domain expansion.
