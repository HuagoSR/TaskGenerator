...
R10 Scenario-First is the active research boundary. R10.0 offline contracts, R10.1 public seed admission and R10.2 teacher-only Bible compilation are implemented: four admitted public Work Seeds, two professional rule sets, six V1 contracts, report-only validators and four admitted Scenario Bibles. R10.3 readiness exposed that a deterministic projection of narrative Bibles is the wrong abstraction. Keep those Bibles frozen; do not build a structured Bible V2 or fabricate files from prose. The R10.3 Skill foundation and curation are implemented: two curated professional Skill packages, a thin catalog, progressive loader and source-bound DeepSeek draft batch exist, while the historical Registry remains unchanged. R10.5's public Codex/Terra probe passed under the restricted outer-Docker/non-nested-Shell configuration. Two historical private cohorts remain `incomplete` interface diagnostics. The automatic Skill Compiler then produced two DeepSeek-reviewed professional Skills; the final four-session common-base A/B cohort passed static admission and both condition-blind reviews, yielding `skill_effect_supported`. R10.6/R10.7A compiled four user-reviewed, statically admitted tasks. R10.7B was behaviorally admitted but later froze because a procurement teacher anchor incorrectly treated 3/40 as within 5%. R10.8A corrected that teacher-only anchor and completed a fresh judge-only regrade: local Codex 0.149.1 provided GPT Judge, official DeepSeek/OpenCode provided DeepSeek Judge, and all four replacement assessments completed. The pilot is again `behaviorally_admitted` with low model separation. R10.8B-1 then classified the frozen pilot as `compiler_revision_candidate`: two saturated tasks, one near tie and one Judge-ambiguous task. R10.8B-2 produced two statically admitted revision tasks and repaired the Huago return path so a fresh public gate passed both Tuzi Codex and DeepSeek/OpenCode. In the isolated private run the revenue task remained a 0.875/0.875 near tie, while the Tuzi price-task session ended in a provider router disconnect before delivery. The experiment therefore closed as `incomplete`; it does not authorize six/ten-task expansion, registry mutation, release activation, training or promotion.
