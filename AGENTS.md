...
- F. Training-data readiness — a governed LLM semantic-validity stage is implemented; the user authorized external 8+8 calibration, but current runtime policy blocked the upload and no provider call occurred.
...
Milestones D and E are complete. Milestone F finance production produced 60/60 candidate-ready tasks. The governed evaluation of 30 tasks is complete with 120/120 solver processes returned, 116 valid deliveries, 116 primary grades, 63 audit grades, four `gpt-4o-mini` non-deliveries, and no OOM or provider failure. A fixed 8-task human-style diagnostic then found all three task templates need candidate-visible contract repair and all eight sampled rubrics need fact-centered revision. Milestone F2 has now added a candidate-blind `semantic_validation` stage between task generation and production review. The user authorized its external 8+8 calibration, but current runtime policy blocked the upload and no provider call occurred. Until real calibration passes, keep the gate diagnostic, treat legacy tasks as semantically unevaluated, and do not authorize further task evaluation, RL training, default-chain promotion, or broad multi-domain expansion.
