...
- F. Training-data readiness — F4.2 completed a fresh server-side source-to-task validation, but F4.3 real rw-task execution reopened task quality: only 15/32 solver combinations produced actual deliverable files, and three tasks had systematic delivery failure caused by prompt/template/deliverable-contract inconsistency. Fix the output-contract gate before route comparison, RL, or default-chain promotion.
...
Milestones D and E are complete. Milestone F produced 60/60 candidate-ready tasks, followed by the governed 30-task evaluation and 8-task diagnosis. F2/F3/F3.1 remain historical evidence of over-fragmented semantic review. F4 repaired a fixed cohort and F4.2 produced eight fresh server tasks, but F4.3 showed that semantic validity and export compatibility did not guarantee actual submission: Slots 2/5/8 had systematic delivery failure and the old summary overstated process completion as delivery. The immediate boundary is a blocking prompt/template/deliverable-path consistency gate and a small revalidation of those three slots. Keep `evidence_to_deliverable` experimental; do not start RL, route comparison, or default-chain promotion before this is resolved.
