...
- F. Training-data readiness — F3 generator-owned semantic contracts passed deterministic offline calibration, but real repaired-task calibration stopped after only 1/3 passed and Tuzi secondary capacity failed; decision is `f3_semantic_gate_hold_for_redesign_and_secondary_capacity`.
...
Milestones D and E are complete. Milestone F finance production produced 60/60 candidate-ready tasks, followed by the governed 30-task evaluation and 8-task human-style diagnosis. F2 remains a historical hold. F3 creates generator-owned `design/resolved/verified` contracts and passed deterministic offline calibration, but real external calibration did not pass: old flawed tasks were blocked `8/8`, while only 1 of the first 3 repaired tasks passed and Tuzi later returned insufficient quota. The `7/8` threshold became unreachable, so the campaign stopped. Keep the gate diagnostic, treat legacy tasks as semantically unevaluated, do not run the 5-task external smoke or 60-task audit, and do not authorize RL training or default-chain promotion.
